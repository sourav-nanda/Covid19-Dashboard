...
time_daily[['Dailyconfirmed', 'Dailydeceased', 'Dailyrecovered']] = time_daily[
    ['Dailyconfirmed', 'Dailydeceased', 'Dailyrecovered']].astype('int64')
...
def country_data():
    ...
    try:
        ...
        st.write(
            'It seems theres a connnection error :confused: not your fault though :smiley:, just refresh the page.',
            Unsafe_allow_html=True)
    finally:
        r = json.loads(r)
        da = pd.DataFrame(r)
...
tiles = ['CartoDB positron', 'Stamen Terrain', 'Stamen Toner', 'Stamen Watercolor', 'OpenStreetMap',
         'CartoDB dark_matter']
...
lat = [19.300229, 10.959789, 16.630049, 14.805643, 28.625976, 27.598203, 23.255716, 25.429921, 18.1124, 22.291606,
       26.404705, 26.452103, 20.397666, 29.153938, 11.666667, 9.494647, 31.187630, 34.147328, 23.347768, 21.233333,
       30.324427, 15.498289, 23.836049, 11.933812, 24.808053, 31.104423, 25.674673, 27.102349, 23.254688, 34.17,
       30.736292, 20.564715, 25.573987, 27.325739, 23.736701, 10.566667]
lon = [75.346739, 78.151884, 79.793588, 76.738056, 77.215747, 79.040305, 88.363044, 85.245790, 79.0193, 72.587265,
       92.798934, 75.138671, 85.326136, 76.853736, 77.402892, 76.331108, 75.753327, 74.631323, 86.185448, 81.633333,
       78.033922, 73.824541, 91.279386, 79.829792, 93.944203, 77.166623, 94.110988, 93.692047, 92.750000, 77.58,
       76.788398, 71.910156, 91.896807, 88.612155, 92.714596, 72.616667]

html = folium.Html(
    '<div style="padding:10px; width:180px; height:50px; background:deepskyblue; display:block; text-align:left;">{stat}</div>',
    script=True)
